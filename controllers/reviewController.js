...
const createReview= async(req,res)=>{
    ...
    if(!isValidProduct) throw new CustomError.NotFoundError(`No product with ID:${productId} exists`)
    const alreadySubmitted= await Review.findOne({product:productId, user:req.user.userId})
    if(alreadySubmitted) throw new CustomError.BadRequestError('Review has already been submitted for this product')
...
const getSingleReview= async(req,res)=>{
    ...
    if(!review) throw new CustomError.NotFoundError(`No review with ID:${reviewId} exists`)
...
const deleteReview= async(req,res)=>{
    ...
    checkPermissions(req.user, review.user)
    await review.remove()
    res.status(StatusCodes.OK).json({msg:'Review has been successfully deleted!!'})
}
